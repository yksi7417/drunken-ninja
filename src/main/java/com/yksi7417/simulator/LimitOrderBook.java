...
public class LimitOrderBook {
	...
	private List< Consumer<List<Trade>> > tradeConsumers = new ArrayList<>();
	...
	private LimitOrder matchLimitOrder(LimitOrder limitOrder, PriorityQueue<LimitOrder> oppsSideQueue) {
		...
		while (remainingOrder.getQty() > 0) {
			...
			if (topOrder.getQty() == 0)
				oppsSideQueue.remove();
		}
		
		if (!trades.isEmpty())
			tradeConsumers.forEach(tradeConsumer -> tradeConsumer.accept(trades));
	 ...
	 */
	public void subscribeTradeEvent(Consumer<List<Trade>> tradeConsumer) {
	 ...
	 */
	public void unsubscribeTradeEvent(Consumer<List<Trade>> tradeConsumer) {
